...
%***************** Range compressed data for one target ******************
...
% Distance correction for each column0
...
for row=1:rows
    azimuth2(row,:)=interp1(freq_azimuth,abs(azimuth),linspace(min(freq_azimuth),max(freq_azimuth),length(waypoints)));
end
...
azimuth2=interp1(freq_azimuth,abs(azimuth),linspace(min(freq_azimuth),max(freq_azimuth),length(waypoints)));

[min,posin] = find(RMC, 1, 'first');
[min2,posout]  = find(RMC, 1, 'last');
meanRMC=(posout+posin)/2;
[val,pos] = max(azimuth2);
shift=abs(meanRMC-pos);
